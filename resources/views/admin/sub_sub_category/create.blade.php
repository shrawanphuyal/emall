  ...
  <form action="{{$edit?route($routeType.'.update',$sub_category):route($routeType.'.store')}}"
    ...
    <div class="card">
      ...
      <div class="card-header card-header-text" data-background-color="green">
        <h4 class="card-title">{{ $edit?'Edit':'Add a New' }} <b>Sub-sub category</b></h4>
      </div>
        ...
        <div class="row">
          <div class="{{ $edit?'col-md-12':'col-md-4' }}">
            ...
            <div class="form-group">
              <label class="control-label" for="category_id">Sub Category</label>
              <select name="category_id"
                ...
                @foreach($categories as $category)
                  <option value="{{$category->id}}"
                      {{ $edit?(($category->id==$sub_category->sub_category_id)?'selected':''):(($category->id==request()->sub_category_id)?'selected':'') }}>{{$category->name}}</option>
                @endforeach
              </select>
...
@push('script')
  ...
  <script>
    $(document).ready(function () {
      ...
      html += ' <input type="text" class="form-control" name="name[]" placeholder="Other Sub sub category name" required="true" />';
